...
class PhotolithographyChallengeWorkflow(AbstractWorkflow):
    """
    Photolithography challenge implemented into the digital twin.

    After initialization, the workflow can be used to run a simulation by calling run().    
    """

    command_path: str = dataclasses.field(default_factory = lambda: str(pathlib.Path(__file__).parent.absolute() / 'workflow_photolithography.sh'))
